...
export default function PutAway() {
    ...
    const fetchTasks = async () => {
        try {
            ...
            const res = await fetch(
                "http://localhost:8000/putaway-read",
                {
    ...
    const loadLocations = async (task_id) => {
        const res = await fetch(`http://localhost:8000/putaway-locations/${task_id}`);
        const data = await res.json();
    ...
    const handleUseMax = async (task, grn) => {
        ...
        try {
            const res = await fetch(`http://localhost:8000/putaway-max-qty/${task.task_id}`);
            const data = await res.json();
    ...
    const handleConfirm = async (task) => {
        ...
        try {
            const res = await fetch(
                `http://localhost:8000/putaway-confirm/${task.task_id}`,
                {
                    method: "POST",
                    headers: getAuthHeaders(),
                    body: JSON.stringify({
